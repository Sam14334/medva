...
foreach ($pdf in $pdfFiles) {
    try {
        ...
        $file = [System.WindowsRuntimeSystemExtensions]::AsTask($asyncFile).GetAwaiter().GetResult()

        $asyncDoc = [Windows.Data.Pdf.PdfDocument]::LoadFromFileAsync($file)
        $doc = [System.WindowsRuntimeSystemExtensions]::AsTask($asyncDoc).GetAwaiter().GetResult()
        ...
        [System.WindowsRuntimeSystemExtensions]::AsTask($asyncRender).GetAwaiter().GetResult() | Out-Null
