...
$asTaskNonGeneric = [System.WindowsRuntimeSystemExtensions].GetMethods() | Where-Object { $_.Name -eq "AsTask" -and -not $_.IsGenericMethod -and $_.GetParameters().Length -eq 1 } | Select-Object -First 1

function AwaitWinRTVoid($asyncOp) {
...
foreach ($pdf in $pdfFiles) {
    try {
        ...
        AwaitWinRT $asyncRender ([uint32]) | Out-Null
