...
$pdfFiles = Get-ChildItem -Path "c:\Users\Mark Samia\OneDrive\Desktop\Codes-html\medva\assets\Resume-CV" -Filter "*.pdf"
...
foreach ($pdf in $pdfFiles) {
    try {
        ...
        for ($i = 0; $i -lt $doc.PageCount; $i++) {
            $pngPath = [System.IO.Path]::Combine($pdf.DirectoryName, "$($pdf.BaseName)_page_$($i+1).png")
            $page = $doc.GetPage($i)
            ...
            $stream.Dispose()
            $page.Dispose()
            Write-Host "Successfully saved page $($i+1) to $pngPath"
        }
    } catch {
